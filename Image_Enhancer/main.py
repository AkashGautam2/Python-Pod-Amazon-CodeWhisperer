#Create a streamlit project to take input as an image # and display the image.
import streamlit as st
import cv2
import numpy as np
st.title("Image Enhancer") # Changed name
...
#Write a function to resize image which takes input as image, width and height and returns width, height and resized image
    ...
    def brightness_increase(self,image, percentage=30):
        ...
        return final_img
    #Write a function that will take input image and percentage and de# crease the brightness of image with that percentage
    ...
    def default(self):
        uploaded_file = st.file_uploader("Choose an image...", type=['png', 'jpg', 'jpeg'])#Changed type
        if uploaded_file is not None:
            image = np.array(bytearray(uploaded_file.read()), dtype=np.uint8)
            download_image = cv2.imdecode(image,  cv2.IMREAD_COLOR)
    #Create variables as width and height and take input as text (used number, but it creates a problem to convert to int)
...
#Call default function with argument main
obj=ImageEnhancer()
obj.default()


# write test cases for st.text_input() function
